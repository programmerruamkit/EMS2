<?php include('db.php');
    ...
    {
        // ตรวจสอบว่า URL มี /survey/ หรือไม่
        $current_uri = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '';
        
        // แก้ไขส่วนนี้ในไฟล์ connect.php
        // ถ้า URL มี /survey/ ให้อนุญาตให้เข้าได้ (ไม่ redirect)
        if (strpos($current_uri, '/survey/') !== false) {
            // ตรวจสอบว่าเป็น survey หน้าแรกหรือไม่
            if (strpos($current_uri, 'survey/customer_survey.php') !== false) {
                // ดึง query string ทั้งหมด
                $query_string = isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : '';
                
                // สร้าง URL ใหม่ไป mobile version
                $mobile_url = '../MOBILE/pages/survey/customer_survey.php';
                if (!empty($query_string)) {
                    $mobile_url .= '?' . $query_string;
                }
                
                header('Location: ' . $mobile_url);
                exit();
            }
            // ถ้าเป็นหน้า survey อื่นๆ ให้ผ่านได้
        } else {
            // ถ้าไม่ใช่หน้า survey ให้ redirect ไป mobile ตามเดิม
