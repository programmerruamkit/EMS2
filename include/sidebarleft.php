	...
	<?php 
		...
		if($menu_id!="14"){
			...
			while($result_menumain = sqlsrv_fetch_array($query_menumain, SQLSRV_FETCH_ASSOC)){	?>	
				<span class="" style="font-weight:bold;"><li><font size='3px'><b><?=$result_menumain['MN_NAME']?></b></font></li></span>
<?php 		}
			...
			while($result_menusub = sqlsrv_fetch_array($query_menusub, SQLSRV_FETCH_ASSOC)){ ?>
				<li onclick="loadViewdetail('<?=$path.$result_menusub['MN_URL']?>','<?=$result_menusub['MN_ID']?>')" style='cursor:pointer'><i class='gg-chevron-right'></i>&nbsp;&nbsp;&nbsp;<font size='3px'><?=$result_menusub['MN_NAME']?></font>
					...
					<?php } ?>
					<?php if($menu_id==28){ ?>
						<span id="checksideassign"></span>
					<?php } ?>
					<?php if($menu_id==32){ ?>
						<span id="checksidework"></span>
					<?php } ?>
				</li>
<?php 		}
		}else{
...
<?php 		}
			...
			$query_menusub = sqlsrv_query($conn, $sql_menusub);
			while($result_menusub = sqlsrv_fetch_array($query_menusub, SQLSRV_FETCH_ASSOC)){ ?>
				<li onclick="loadViewdetail('<?=$path.$result_menusub['MN_URL']?>','<?=$result_menusub['MN_ID']?>')" style='cursor:pointer'><i class='gg-chevron-right'></i>&nbsp;&nbsp;&nbsp;<font size='3px'><?=$result_menusub['MN_NAME']?></font></li>
<?php 		}
		}
?>
</ul>
