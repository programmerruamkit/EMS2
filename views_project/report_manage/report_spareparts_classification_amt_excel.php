<?php
    ...
    // รับค่า group และ spareparts จาก parameter
    $group = isset($_GET['group']) ? $_GET['group'] : '';
    $spareparts = isset($_GET['spareparts']) ? $_GET['spareparts'] : '';
    $status = isset($_GET['status']) ? $_GET['status'] : ''; // เพิ่มบรรทัดนี้
    ...
    // กำหนดชื่อตาม spareparts
    $spareparts_names = array(
        'due_soon' => 'ครบกำหนดเปลี่ยน (0-5 วัน)',
        'approaching' => 'ใกล้ถึงกำหนด (5-15 วัน)', 
        'overdue' => 'เกินกำหนดแล้ว'
    );
    $spareparts_title = isset($spareparts_names[$status]) ? $spareparts_names[$status] : ''; // เปลี่ยนจาก $spareparts เป็น $status

    $spreadsheet = new Spreadsheet();
    
    // ดึงข้อมูลอะไหล่ที่เลือกเท่านั้น
    ...
    while($sparepart = sqlsrv_fetch_array($query_spareparts, SQLSRV_FETCH_ASSOC)) {
        ...
        while($vehicle = sqlsrv_fetch_array($query_vehicles, SQLSRV_FETCH_ASSOC)) {
            ...
            // แสดงข้อมูลเฉพาะที่ตรงเงื่อนไข spareparts
            if ($include_record) {
                $no++;
                $has_data = true;
                $sheet_has_data = true;
    ...
    $safe_spareparts_name = preg_replace('/[^\w\s-]/', '', $spareparts_title);
    $filename = "Equipment_Sparepart_Report_" . str_replace(array(' ', '-', '/', '(', ')'), '', $safe_group_name);
    if ($safe_spareparts_name) {
        $filename .= "_" . str_replace(array(' ', '-', '/', '(', ')'), '_', $safe_spareparts_name);
    }
