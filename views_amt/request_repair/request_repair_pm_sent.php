<?php
  ...
  $CTM_GROUP=$_GET['ctmgroup'];
    ...
    $VEHICLEREGISNUMBER=$result_car['VEHICLEREGISNUMBER'];
    $THAINAME=$result_car['THAINAME'];
    if($result_car['AFFCOMPANY']=="RKS"||$result_car['AFFCOMPANY']=="RKR"||$result_car['AFFCOMPANY']=="RKL"){     
        $field="VEHICLEREGISNUMBER = '$VEHICLEREGISNUMBER'";
    }else if($result_car['AFFCOMPANY']=="RRC"||$result_car['AFFCOMPANY']=="RCC"||$result_car['AFFCOMPANY']=="RATC"){
    ...
    if($CTM_GROUP=="cusout"){
      ...
      $sql_maxmil = "SELECT TOP 1 MILEAGENUMBER AS MAXMILEAGENUMBER,VEHICLEREGISNUMBER,CREATEDATE FROM TEST_MILEAGE_PM WHERE ACTIVESTATUS = 1  AND MILEAGENUMBER <> '0' AND $field ORDER BY MILEAGEID DESC ";
    }else{
      // $sql_maxmil = "SELECT TOP 1 * FROM vwMILEAGE WHERE VEHICLEREGISNUMBER = '".$result_car['VEHICLEREGISNUMBER']."' ORDER BY CREATEDATE DESC ";
      $sql_maxmil = "SELECT TOP 1 MAXMILEAGENUMBER,VEHICLEREGISNUMBER,THAINAME,CREATEDATE FROM TEMP_MILEAGE WHERE MAXMILEAGENUMBER > '0' AND $field ORDER BY CREATEDATE DESC ";
    }
    $params_maxmil = array();
