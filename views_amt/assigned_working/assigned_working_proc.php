<?php
	...
	if($target=="save_successjob_bm"){
		...
		// แจ้งเตือนเทเลแกรม-OPEN-------------------------------------------------------------------------------
			$stmt_telegram = "SELECT * FROM SETTING WHERE ST_TYPE = '36' AND ST_STATUS = 'Y' AND ST_AREA = '$SESSION_AREA'";
		...
		$RPRQ_NAP_NEW_CODE = $RPRQ_CODE;	
		$sql_nap = "UPDATE REPAIRREQUEST_NONAPPROVE SET 
				RPRQ_NAP_STATUS = ?
				WHERE RPRQ_NAP_NEW_CODE = ?";
		$params_nap = array('SUCCESS',$RPRQ_NAP_NEW_CODE);		
	...
	if($target=="imagerepair"){
		...
		if( !empty( $fileToUpload_tmp_name ) && !empty( $fileToUpload_name ) ) {
			...
			for( $i=0; $i<count( $fileToUpload_tmp_name ); $i++ ) {
				...
				if( move_uploaded_file( $fileToUpload_tmp_name[$i], $path."../uploads/requestrepair/".$RPATIM_IMAGES) ) {	
	...
	if($proc=="delete"){
		...
		@unlink($path."../uploads/requestrepair/".$RPATIM_IMAGES);
