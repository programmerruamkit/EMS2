<?php
	...
	$mn_id=$_GET["id"];
	if($proc=="edit"){
		$stmt = "SELECT * FROM MENU WHERE MN_CODE = ? AND NOT MN_STATUS='D'";
		$params = array($mn_id);	
		$query = sqlsrv_query( $conn, $stmt, $params);	
		$result_edit_menu = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC);
    $MN_CODE=$result_edit_menu["MN_CODE"];
	};
	if($proc=="add"){
		$stmt = "SELECT TOP 1 MN_ID FROM MENU WHERE NOT MN_STATUS='D' ORDER BY MN_ID DESC";
		$query = sqlsrv_query( $conn, $stmt);	
		$result_add_menu = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC);
    $MN_ID=$result_add_menu["MN_ID"];
    $CALMN_ID=$MN_ID+1;
	};
	$n=6;
	function RandNum($n) {
...
<script type="text/javascript">
    ...
    var buttonname = $('#buttonname').val();
		var url = "<?=$path?>views_amt/menu_manage/menu_sub_proc.php"; 
		$.ajax({ type: "POST",
			   url: url,
          ...
          Swal.fire({
            ...
            if(buttonname=='add'){
              ...
              log_transac_menu('LA18', '<?=$result_edit_menu["MN_NAME"];?>', '<?=$result_edit_menu["MN_CODE"];?>');
            }
				  	loadViewdetail('<?=$path?>views_amt/menu_manage/menu_sub.php?mn_id=<?php print $GET_MN_ID; ?>&area=<?=$MN_AREA;?>');
				   	closeUI();
				   	// alert(data);
          })
			   }
                ...
                <td class="CENTER"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                  <tr>
                    <?php if($proc=="edit"){ ?>
                      ...
                      <td valign="bottom" class=""><h4>&nbsp;&nbsp;แก้ไขเมนูย่อย</h4></td>
                    <?php }else{ ?>
                      <td width="24" valign="middle" class=""><img src="../images/plus-icon32.png" width="32" height="32"></td>
                      <td valign="bottom" class=""><h4>&nbsp;&nbsp;เพิ่มเมนูย่อยใหม่</h4></td>
                    <?php } ?>
                    </tr>
                </table></td>
                <td class="RIGHT"></td>
                ...
                <form action="#" method="post" enctype="multipart/form-data" name="form_project" id="form_project">
                  <table width="100%" cellpadding="0" cellspacing="0" border="0" class="default">
                      ...
                      <tr align="center" id="22" height="25px">
                        <td width="30%" height="35" align="right" class="ui-state-default"><strong>ชื่อเมนู  :</strong></td>
                        <td width="70%" height="35" align="left" class="bg-white">
                          <div class="input-control text">
                            <input type="text" name="MN_NAME" id="MN_NAME" class="time" onFocus="$(this).select();" value="<?=$result_edit_menu["MN_NAME"];?>" required>
                          </div>
                        </td>
                        </tr>
                      <tr align="center" height="25px">
                        ...
                        <td height="35" align="left" class="bg-white">
                          <div class="input-control text">
                            <input type="text" class="time" name="MN_SORT" id="MN_SORT" value="<?=$result_edit_menu["MN_SORT"];?>" onKeyUp="handleEnter(this, event, 0);" onBlur="$(this).val(number_format_num($(this).val(),0))" onFocus="onfocus_format(this);" required />
                          </div>
                        </td>
                      </tr>
                      <tr align="center" id="222" height="25px">
                        <td height="35" align="right" class="ui-state-default"><strong>ที่อยู่ไฟล์ :</strong></td>
                        <td height="35" align="left" class="bg-white">
                          <div class="input-control text">
                            <input type="text" name="MN_URL" id="MN_URL" class="time" onFocus="$(this).select();" value="<?=$result_edit_menu["MN_URL"];?>" required>
                          </div>
                        </td>
                      </tr>
                      <tr align="center" height="25px">
                        ...
                        <td height="35" align="left" class="bg-white">
                          <div class="input-control text">
                              <select class="time" onFocus="$(this).select();" style="width: 100%;" name="MN_STATUS" id="MN_STATUS" required>
                                  <option value disabled selected>-------โปรดเลือก-------</option>
                                  <option value="Y" <?php if($result_edit_menu['MN_STATUS']== "Y"){echo "selected";} ?>>เปิดใช้งาน</option>
                                  <option value="N" <?php if($result_edit_menu['MN_STATUS']== "N"){echo "selected";} ?>>ปิดใช้งาน</option>
                              </select>
                          </div>
                        </td>
                      </tr>
                      
                      <input type="hidden" name="area" id="area" value="<?=$MN_AREA;?>" />
                      <input type="hidden" name="MN_ID" id="MN_ID" value="<?=$result_edit_menu['MN_ID'];?>" />
                      <input type="hidden" name="proc" id="proc" value="<?=$proc;?>" />
                      <input type="hidden" name="mn_id" id="mn_id" value="<?=$mn_id; ?>">
                      <input type="hidden" name="MN_PARENT" id="MN_PARENT" value="<?=$GET_MN_ID; ?>">              
                      <tr align="center" height="25px">
                          <td height="35" colspan="2" align="center" >                            
                          <?php if($proc=="edit"){ ?>
                            <input type="hidden" name="mn_code" id="mn_code" value="<?=$result_edit_menu['MN_CODE']; ?>">
