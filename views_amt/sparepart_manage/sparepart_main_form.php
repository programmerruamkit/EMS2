<?php
	...
	if($proc=="edit"){
		$stmt = "SELECT * FROM SPAREPART WHERE SPP_CODE = ? AND NOT SPP_STATUS='D'";
		$params = array($spp_id);	
		$query = sqlsrv_query( $conn, $stmt, $params);	
		$result_edit_sparepart = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC);
    $SPP_CODE=$result_edit_sparepart["SPP_CODE"];
	};
	if($proc=="add"){
		$stmt = "SELECT TOP 1 SPP_ID FROM SPAREPART WHERE NOT SPP_STATUS='D' ORDER BY SPP_ID DESC";
		$query = sqlsrv_query( $conn, $stmt);	
		$result_add_sparepart = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC);
    $SPP_ID=$result_add_sparepart["SPP_ID"];
    $CALSPP_ID=$SPP_ID+1;
	};  
	$n=6;
	function RandNum($n) {
...
<head>
  <script type="text/javascript">
    function save_data() {
      ...
      var url = "<?=$path?>views_amt/sparepart_manage/sparepart_main_proc.php";
      $.ajax({
        type: "POST",
        url: url,
        ...
        success: function (data) {
          Swal.fire({
            ...
            if(buttonname=='add'){
              ...
              log_transac_sparepart('LA18', '<?=$result_edit_sparepart["SPP_NAME"];?>', '<?=$result_edit_sparepart["SPP_CODE"];?>');
            }
                      ...
                      <tbody>
                        <tr align="center" height="25px">
                          ...
                          <td height="35" align="left" class="bg-white">
                            <div class="input-control text">
                              <input type="text" class="time" name="SPP_CODENAME" id="SPP_CODENAME" value="<?=$result_edit_sparepart["SPP_CODENAME"];?>" required />
                            </div>
                          </td>
                        </tr>
                        <tr align="center" id="22" height="25px">
                          ...
                          <td width="70%" height="35" align="left" class="bg-white">
                            <div class="input-control text">
                              <input type="text" name="SPP_NAME" id="SPP_NAME" class="time" onFocus="$(this).select();" value="<?=$result_edit_sparepart["SPP_NAME"];?>" required>
                            </div>
                          </td>
                        </tr>
                        <tr align="center" height="25px">
                          <td height="35" align="right" class="ui-state-default"><strong>จำนวนคงเหลือ :</strong></td>
                          <td height="35" align="left" class="bg-white">
                            <div class="input-control text">
                              <input type="text" class="time" name="SPP_STOCK" id="SPP_STOCK" value="<?=$result_edit_sparepart["SPP_STOCK"];?>" onKeyUp="handleEnter(this, event, 0);" onBlur="$(this).val(number_format_num($(this).val(),0))" onFocus="onfocus_format(this);" required />
                            </div>
                          </td>
                        </tr>
                        <tr align="center" height="25px">
                          ...
                          <td align="left" class="bg-white">
                            ...
                            <label class="container">
                              <input type="radio" name="SPP_LOCAT" id="SPP_LOCAT" value="AMT" <?php if($result_edit_sparepart['SPP_LOCAT']== "AMT"){echo "checked";} ?>>
                              <span class="checkmark">AMT</span>
                            </label>
                            &nbsp;&nbsp;&nbsp;&nbsp;
                            <label class="container">
                              <input type="radio" name="SPP_LOCAT" id="SPP_LOCAT" value="GW" <?php if($result_edit_sparepart['SPP_LOCAT']== "GW"){echo "checked";} ?>>
                        ...
                        <tr align="center" height="25px">
                          ...
                          <td height="35" align="left" class="bg-white">
                            <div class="input-control text">
                              <select class="time" onFocus="$(this).select();" style="width: 100%;" name="SPP_STATUS" id="SPP_STATUS" required>
                                <option value disabled selected>-------โปรดเลือก-------</option>
                                <option value="Y" <?php if($result_edit_sparepart['SPP_STATUS']== "Y"){echo "selected";} ?>>เปิดใช้งาน
                                </option>
                                <option value="N" <?php if($result_edit_sparepart['SPP_STATUS']== "N"){echo "selected";} ?>>ปิดใช้งาน
                                </option>
                              </select>
                            </div>
                          </td>
                        </tr>
                        <input type="hidden" name="SPP_ID" id="SPP_ID" value="<?=$result_edit_sparepart['SPP_ID'];?>" />
                        <input type="hidden" name="proc" id="proc" value="<?=$proc;?>" />
                        <input type="hidden" name="spp_id" id="spp_id" value="<?=$spp_id; ?>">      
                        <tr align="center" height="25px">
                          <td height="35" colspan="2" align="center">
                            <?php if($proc=="edit"){ ?>
                              <input type="hidden" name="spp_code" id="spp_code" value="<?=$result_edit_sparepart['SPP_CODE'];?>">
