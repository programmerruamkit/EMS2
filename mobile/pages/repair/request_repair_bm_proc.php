<?php
	...
	if($proc=="add"){
		...
		$exin1 = $exin[0];
		$exin2 = $exin[1]; 		
		$exinex = explode("-", $exin1);
		$exinex1 = $exinex[0];
		$exinex2 = $exinex[1]; 
		$exinex3 = $exinex[2]; 
		$newexin = $exinex3.'/'.$exinex2.'/'.$exinex1;
		$RPRQ_REQUESTCARDATE = $newexin;
		$RPRQ_REQUESTCARTIME = substr($exin2,0,5);
		$datetimeRequest_out = $_POST["datetimeRequest_out"];		
		$exout = explode("T", $datetimeRequest_out);
		$exout1 = $exout[0];
		$exout2 = $exout[1]; 		
		$exoutex = explode("-", $exout1);
		$exoutex1 = $exoutex[0];
		$exoutex2 = $exoutex[1]; 
		$exoutex3 = $exoutex[2]; 
		$newexout = $exoutex3.'/'.$exoutex2.'/'.$exoutex1;
		$RPRQ_USECARDATE = $newexout;
		$RPRQ_USECARTIME = substr($exout2,0,5);

		$RPRQ_PRODUCTINCAR = $_POST["GOTC"];
		$RPRQ_NATUREREPAIR = $_POST["NTORNNW"];
		$RPRQ_TYPECUSTOMER = "cusin";
		...
		for($inum=0; $inum < count($_POST["RPM_NATUREREPAIR"]); $inum++){
			if($_POST["RPM_NATUREREPAIR"][$inum] != "" ){	
				...
				// แจ้งเตือนไลน์--------------------------------------------------------------------------------
					...
					$stmt_linenoti = "SELECT * FROM SETTING WHERE ST_TYPE = '27' AND ST_STATUS = 'Y' AND ST_AREA = '$AREA'";
					...
					while($result_linenoti = sqlsrv_fetch_array($query_linenoti)){	
						...
						$NOTI_LINE5="ปัญหา : $RPC_DETAIL"."\n";
						$NOTI_LINE6="วันที่/เวลา แจ้งซ่อม : ".$RPRQ_REQUESTCARDATE." ".$RPRQ_REQUESTCARTIME.""."\n";
						$NOTI_LINE7="วันที่/เวลา ต้องการใช้รถ : ".$RPRQ_USECARDATE." ".$RPRQ_USECARTIME.""."\n";
						$NOTI_LINE8="ผู้แจ้ง : ".$RPRQ_REQUESTBY."";   
